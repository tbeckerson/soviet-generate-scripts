...
echo 'blfs-complete file is missing, checking status of BLFS programs'
sleep 1
	if [ "(ls -A /$BLFS_DIR/scripts)" ]; then
        echo 'BLFS build already in progress, continuing'
        sleep 1
        cd $BLFS_DIR/work
        make
    else
        echo 'starting new BLFS build'
        sleep 1
        ...
        sed -i '/unzip -q \.\.\/sqlite-doc/d' $BLFS_DIR/scripts/*sqlite
        sed -i '/install -v -m755 -d \/usr\/share\/doc\/sqlite-/d' $BLFS_DIR/scripts/*sqlite
        sed -i '/cp -v -R sqlite-doc-/d' $BLFS_DIR/scripts/*sqlite
